...
namespace Otter {
    ...
    public class Draw {
        ...
        static internal void Drawable(Drawable graphic, RenderStates states) {
            if (Game.Instance.countRendering) Game.Instance.RenderCount++;

            Target.Draw(graphic, states);
        }

        static internal void Drawable(Drawable graphic) {
            Drawable(graphic, RenderStates.Default);
        }
        ...
        static public void ImageClip(Image image, Rectangle clip, float x = 0, float y = 0) {
            var tempRect = image.SourceRect;
            image.SourceRect = new IntRect(clip.Left, clip.Top, clip.Width, clip.Height);
            image.UpdateTexture();

            image.Render(x, y);

            image.SourceRect = tempRect;
            image.UpdateTexture();
        }
        ...
        static public void ImageWaveX(Image image, int step, float timer, float rate, float amp, float freq, float x = 0, float y = 0) {
            for (var yy = 0; yy < image.Height; yy += step) {
                ...
                ImageClip(image, new Rectangle(0, yy, image.Width, step), x + xx, y + yy);
            }
        }
        ...
        static public void ImageWaveY(Image image, int step, float timer, float rate, float amp, float freq, float x = 0, float y = 0) {
            for (var xx = 0; xx < image.Width; xx += step) {
                ...
                ImageClip(image, new Rectangle(xx, 0, step, image.Height), x + xx, y + yy);
            }
        }
        ...
        static public void Circle(float x, float y, float radius, Color fill = null, Color outline = null, float outlineThickness = 0) {
        ...
        static public void Line(float x1, float y1, float x2, float y2, Color color) {
            VertexArray verticies = new VertexArray(PrimitiveType.Lines);
            verticies.Append(new Vertex(new Vector2f(x1, y1), color.SFMLColor));
            verticies.Append(new Vertex(new Vector2f(x2, y2), color.SFMLColor));
            Drawable(verticies);
        }
        ...
        static public void RoundedLine(float x1, float y1, float x2, float y2, Color color, float thickness) {
            ...
            for(int i = 0; i < rotationSteps; i++) {
                nextPoint = Util.Rotate(nextPoint, 180 / rotationSteps * Util.DEG_TO_RAD);
