...
namespace Otter {
    /// <summary>
    /// Class used for collision detection.  Entities contain Colliders that can check for collisions
    /// which each other.
    /// </summary>
    public class Collider {
        ...
        /// <param name="x">The x position to check.</param>
        /// <param name="y">The y position to check.</param>
        /// <param name="c">The list of colliders to check.</param>
        /// <returns>True if there was a collision.</returns>
        public bool Overlap(float x, float y, List<int> c) {
            return (Collide(x, y, c) != null);
        }
        ...
        internal static bool OverlapTest(Collider first, Collider second) {
            if (first is BoxCollider && second is BoxCollider) {
                ...
                if (Util.PointInRectangle(circle.CenterX, circle.CenterY, box.Left, box.Top, box.Width, box.Height)) {
                    return true;
                }
                    ...
                    for (int i = gridx; i <= gridx2; i++) {
                        for (int j = gridy; j <= gridy2; j++) {
                            if (grid.GetTile(i, j)) {
                                ...
                                if (Util.PointInRectangle((float)line.Line2.PointA.X, (float)line.Line2.PointA.Y, rectX, rectY, grid.TileWidth, grid.TileHeight)) {
                                    return true;
                                }
                                if (Util.PointInRectangle((float)line.Line2.PointB.X, (float)line.Line2.PointB.Y, rectX, rectY, grid.TileWidth, grid.TileHeight)) {
                                    return true;
                                }
                    ...
                    for (int i = gridx; i <= gridx2; i++) {
                        for (int j = gridy; j <= gridy2; j++) {
                            if (grid.GetTile(i, j)) {
                                ...
                                //check is c center point is in the rect
                                if (Util.PointInRectangle(circle.CenterX, circle.CenterY, rectX, rectY, grid.TileWidth, grid.TileHeight)) {
                                    return true;
                                }
                ...
                if (Util.PointInRectangle(point.Left, point.Top, box.Left, box.Top, box.Width, box.Height)) return true;
                return false;
            }
                ...
                //check along line for pixels somehow...
            }
                ...
                //on each tile that is true, check area of pixels on mask

                //then check each pixel against nearby tiles as rectangles
        ...
        public void AddTag(params int[] tags) {
            foreach (int t in tags) {
                AddTag(t);
            }
        }
        ...
        public void RemoveTag(params int[] tags) {
            foreach (int t in tags) {
                RemoveTag(t);
            }
        }
        ...
        public virtual float CenterX {
            get { return X + Entity.X + OriginX + Width * 0.5f; }
            private set { }
        }

        public virtual float CenterY {
            get { return Y + Entity.Y + OriginY + Height * 0.5f; }
            private set { }
        }
